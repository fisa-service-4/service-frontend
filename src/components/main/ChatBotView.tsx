...
import {ArrowRight, Menu, PlusCircle, Trash2, X} from 'lucide-react';
...
function AiCard({content}: { content: string }) {
    ...
    return (
        <div
            className={`max-w-[80%] rounded-2xl overflow-hidden text-sm ${isComplete ? 'bg-primary-50 border border-primary-100' : 'bg-gray-100'}`}>
            <div
                className={`px-4 py-3 font-semibold text-gray-900 ${isComplete ? 'border-b border-primary-100' : 'border-b border-gray-200'}`}>
                {header}
            </div>
            <div className="px-4 py-3 space-y-2">
                {items.map((item, i) => (
                    <div key={i} className="flex justify-between items-center">
                        ...
                        <span
                            className={`font-semibold text-sm ${isComplete ? 'text-primary-700' : 'text-gray-900'}`}>{item.value}</span>
                    </div>
                ))}
            </div>
            {footer && (
...
export default function ChatBotView({onClose}: ChatBotViewProps) {
    ...
    const loadSessions = useCallback(async () => {
        try {
            ...
            // 백엔드가 Page 객체나 DTO 주머니 형태로 내려줄 때를 대비한 안전 코드 추가
    ...
    const loadSession = async (session: SessionSummary) => {
        ...
        try {
            ...
            // 백엔드에서 통배열로 주든, content/messages 키값 안에 넣어서 주든 모두 대응할 수 있게 파싱 방어선 구축
                            ...
                            {sessions.length > 0 && (
                                <div>
                                    ...
                                    <div className="space-y-1 overflow-y-auto">
                                        {sessions.map((s) => (
                                            <div key={s.sessionId}
                                                 className="flex items-center justify-between group rounded-lg hover:bg-slate-800 transition-colors px-2">
                                                <button
                                                    onClick={() => loadSession(s)}
                                                    className="flex-1 text-left text-sm text-slate-200 py-2 truncate"
                                                >
                                                    {s.title || '이름 없는 대화'}
                                                </button>
                                                <button
                                                    onClick={(e) => deleteSession(s.sessionId, e)}
                                                    className="text-slate-500 hover:text-rose-400 p-1 md:opacity-0 group-hover:opacity-100 transition-opacity"
                                                >
                                                    <Trash2 size={14}/>
                                                </button>
                                            </div>
                        ...
                        >
                            {msg.role === 'ai' && (
                                msg.content.startsWith('💰') || msg.content.startsWith('✅') ? (
                                    ...
                                    <div
                                        className="max-w-[75%] px-4 py-3 rounded-3xl rounded-bl-sm text-sm leading-relaxed whitespace-pre-line bg-gray-100 text-gray-900">
                                        {msg.content}
                                    </div>
                                )
                            )}
                            {msg.role === 'user' && (
                                <div
                                    className="max-w-[75%] px-4 py-3 rounded-3xl rounded-br-sm text-sm leading-relaxed whitespace-pre-line bg-primary-500 text-white">
                                    {msg.content}
                                </div>
                            )}
                        </div>
